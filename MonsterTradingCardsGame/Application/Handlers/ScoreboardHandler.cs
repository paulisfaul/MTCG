...
namespace MonsterTradingCardsGame.Application.Handlers
{
    public class ScoreboardHandler:Handler
    {
        ...
        public ScoreboardHandler(UserService userService, AuthenticationService authenticationService)
        {
            _userService = userService;
            _finalRouteHandlerFunctions.Add("/", (HandleScoreboard, authenticationService.AuthorizePlayer));
        }

        public async Task<bool> HandleScoreboard(HttpSvrEventArgs e, User user)
        {
            if (e.Method == "GET")
            {
                var result = await _userService.GetScoreboard();
