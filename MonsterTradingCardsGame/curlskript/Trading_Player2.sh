...
curl -i -X POST http://localhost:10001/api/tradings/ --header "Content-Type: application/json" --header "Authorization: $token_altenhof" -d "{\"CardId\":\"$offered_id\", \"CardType\":\"Monster\" , \"MinDmg\":10, \"AutomaticAccept\":true}"

response=$(curl -i -X GET http://localhost:10001/api/tradings/ --header "Content-Type: application/json" --header "Authorization: $token_altenhof")
...
curl -i -X POST http://localhost:10001/api/tradings/$trading_id --header "Content-Type: application/json" --header "Authorization: $token_altenhof" -d "{\"CardId\":\"$accepting_id\"}"


#curl -i -X DELETE http://localhost:10001/api/tradings/db2f49cd-9c59-4851-8846-c7ed0d02ec52  --header "Content-Type: application/json" --header "Authorization: $token_altenhof"